...
class TestGetWriteClient:
    def test_returns_local_clients(self, monkeypatch):
        local = FakeZotero()
        write_zot = FakeZotero()
        monkeypatch.setattr("zotero_mcp.client.get_zotero_client", lambda: local)
        monkeypatch.setattr("zotero_mcp.client.get_local_write_client", lambda: write_zot)
        monkeypatch.setattr("zotero_mcp.client.get_active_library", lambda: {})
        read_zot, out_write = server._get_write_client(DummyContext())
        assert read_zot is local
        assert out_write is write_zot

    def test_library_override_propagated(self, monkeypatch):
        local = FakeZotero()
        write_zot = FakeZotero()
        write_zot.library_id = "personal"
        ...
        _, out_write = server._get_write_client(DummyContext())
        assert out_write.library_id == "group123"
        assert out_write.library_type == "groups"
