...
def test_update_note_replaces_content(monkeypatch):
    ...
    monkeypatch.setattr("zotero_mcp.client.get_zotero_client", lambda: fake)
    monkeypatch.setattr("zotero_mcp.client.get_local_write_client", lambda: fake)
    monkeypatch.setattr("zotero_mcp.utils.is_local_mode", lambda: False)
...
class FakeZoteroForDeleteNote:
    ...
    def _write(self, method, url, **kwargs):
        ...
        if method == "PATCH":
            return self.client.patch(url=url, headers=headers, content=content)
        return FakePatchResponse(204)
