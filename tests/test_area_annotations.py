...
def test_create_area_annotation_happy_path(monkeypatch, fake_zot):
    ...
    monkeypatch.setattr("zotero_mcp.client.get_local_write_client", lambda: fake_zot)
...
def test_create_area_annotation_requires_write_authorization(monkeypatch):
    def boom():
        raise RuntimeError("No local Zotero write key")
    monkeypatch.setattr("zotero_mcp.client.get_local_write_client", boom)
    ...
    assert "without local write authorization" in result
