...
class FakeZoteroForAttach(FakeZotero):
    """FakeZotero extended with upload_attachments recording."""

    def __init__(self):
        super().__init__()
        self.attachments = []

    def upload_attachments(self, payload, parentid=None, **kwargs):
        files = [(p.get("title", ""), p.get("filename", "")) for p in payload]
        self.attachments.append({"files": files, "parentid": parentid, "payload": payload})
        title = payload[0].get("title", "") if payload else ""
        return {
            "success": [{"key": "ATCH0001", "title": title}],
            "failure": [],
            "unchanged": [],
        }
    ...
    def test_upload_failure_entries_not_reported_as_success(
        ...
        class FailUpload(FakeZoteroForAttach):
            def upload_attachments(self, payload, parentid=None, **kwargs):
                return {
                    "success": [],
                    "failure": [{"title": payload[0].get("title", ""), "code": 400}],
                    "unchanged": [],
                }
