...
def _download_attachment_for_processing(
    ...
    *,
    ...
    return (
        ...
        f"Attempted sources:\n{error_details}\n\n"
        "Possible solutions:\n"
        "- Ensure local Zotero desktop is running\n"
        "- Ensure the attachment file exists in local Zotero storage"
    )


def _get_note_write_client(op_description: str):
    """Return (client, None) or (None, error_msg) for note/annotation write operations."""
    try:
        zot = _client.get_local_write_client()
        _helpers.apply_library_override(zot, _client.get_active_library())
        return zot, None
    except Exception as e:
        return None, f"Error: Cannot perform {op_description} without local write authorization: {e}"
...
def create_note(
    ...
    *,
    ...
    try:
        ...
        # Create note using local write client
        zot, err = _get_note_write_client("creating notes")
        if err:
            return err
...
def _create_highlight_annotation(
    ...
    *,
    ...
    try:
        ...
        zot, err = _get_note_write_client("creating annotations")
        if err:
            return err
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ctx.info("Creating annotation via local API...")
            result = zot.create_items([annotation_data])
...
def create_area_annotation(
    ...
    *,
    ...
    try:
        ...
        local_client = _client.get_local_zotero_client()
        zot, err = _get_note_write_client("creating area annotations")
        if err:
            return err
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ctx.info("Creating area annotation via local API...")
            result = zot.create_items([annotation_data])
...
def get_page_layout(
    ...
    *,
    ...
    try:
        ...
        meta_client = _client.get_local_zotero_client() or _client.get_zotero_client()
        if meta_client is None:
            return (
                "Error: Local Zotero instance is not accessible. "
                "Ensure Zotero is running on this computer."
            )
