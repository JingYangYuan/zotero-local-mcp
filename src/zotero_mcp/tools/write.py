    ...
    def template_fn(zot_type: str) -> dict:
        if zot_type not in cache:
            with zotero_api_lock():
                cache[zot_type] = _helpers.item_template(write_zot, zot_type)
...
def _add_from_embedded_metadata(
    ...
    template = dict(_helpers.item_template(write_zot, zot_type))
    ...
    *,
    ...
    try:
        ...
        with _helpers.identifier_lock("url", url), zotero_api_lock():
            ...
            template = _helpers.item_template(write_zot, "webpage")
            template["url"] = url
            template["title"] = url
            template["accessDate"] = ""
...
def _add_by_arxiv(arxiv_id, collections, tags, write_zot, ctx, attach_mode="auto",
    ...
    with _helpers.identifier_lock("arxiv", arxiv_id), zotero_api_lock():
        ...
        template = _helpers.item_template(write_zot, "preprint")
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with open(filepath, "wb") as f:
                    for chunk in pdf_resp.iter_content(chunk_size=8192):
                        f.write(chunk)
                attach_ok, suffix, _key = _helpers._attach_and_verify(
                    write_zot,
                    filename,
                    filepath,
                    item_key,
...
def add_by_isbn(
    ...
    *,
    ...
    try:
        ...
        template = _helpers.item_template(write_zot, "book")
        item_data = dict(template)
        if meta.get("title"):
    ...
    try:
        ...
        if item_type is not None:
            ...
            if old_item_type != item_type:
                try:
                    new_template = _helpers.item_template(write_zot, item_type)
                except Exception as e:
...
def delete_item(
    ...
    *,
    ...
    try:
        ...
        resp = _helpers.trash_item(write_zot, item)
    ...
    *,
    ...
    try:
        ...
        if extracted_doi:
            ...
            # Create a basic item
            template = _helpers.item_template(write_zot, item_type)
        ...
        try:
            ...
            attach_info = (
                f"File attached: {display_name}" + suffix
                if attach_ok
                else f"Item created but file attachment FAILED: {suffix}"
            )
        except Exception as e:
...
@mcp.tool(
    ...
    description=(
        ...
        "content (MD5), nothing is re-uploaded. Requires local Zotero write "
        "authorization. Files are copied into local Zotero storage; "
        "subsequent syncing is determined by Zotero desktop settings. Run "
...
@mcp.tool(
    ...
    description=(
        ...
        "Requires local Zotero write authorization. Attached files are "
        "copied into local Zotero storage; subsequent syncing is determined "
        "by Zotero desktop settings. Run "
