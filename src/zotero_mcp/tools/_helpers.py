...
def _get_write_client(ctx):
    """Return (read_client, write_client) for local operations.

    Both clients connect strictly to the local Zotero instance.
    write_client is authorized with the local Zotero write key.
    """
    read_zot = _client.get_zotero_client()
    write_zot = _client.get_local_write_client()
    apply_library_override(write_zot, _client.get_active_library())
...
def item_template(zot, item_type: str, link_mode: str | None = None) -> dict:
    """Return a blank item template for *item_type*, compatible with local Zotero.

    Builds the template using zot.item_type_fields(item_type) or zot.item_template(item_type),
    and populates itemType, creators=[], tags=[], collections=[], relations={}.
    For attachments, populates linkMode, title, filename, contentType, charset, note.
    """
    ...
    fields = None
    if hasattr(zot, "item_type_fields"):
        try:
    ...
    if isinstance(fields, list):
        for f in fields:
            f_name = f.get("field") if isinstance(f, dict) else str(f)
            if f_name and f_name not in template:
                template[f_name] = ""
    elif hasattr(zot, "item_template"):
        tmpl = zot.item_template(item_type)
        if isinstance(tmpl, dict):
            template.update(tmpl)

    for base_field in ("title", "date", "abstractNote", "url", "extra"):
        if base_field not in template:
            template[base_field] = ""
...
def _attach_and_verify(
    write_zot, filename, file_path, parent_key, ctx, content_type=None, read_zot=None
):
    """Upload *file_path* onto *parent_key* via local Zotero, confirming the file landed.

    Returns ``(ok, suffix, attachment_key)``.
    """
    if not os.path.exists(file_path):
        return False, f"File does not exist: {file_path}", None

    detected_content_type = (
        content_type or _guess_content_type(filename) or "application/octet-stream"
    )
    payload = {
        "itemType": "attachment",
    ...
    try:
        attach_result = write_zot.upload_attachments([payload], parentid=parent_key)
    except Exception as exc:
        return False, f"Local upload failed: {exc}", None

    attachment_key = _extract_attachment_key(attach_result)
    if not attachment_key:
        failures = (
            attach_result.get("failure") if isinstance(attach_result, dict) else None
        )
        reason = (
            f"upload rejected by local Zotero: {failures}"
            if failures
            else "no attachment key returned"
        )
        return False, reason, None

    reader_client = read_zot or write_zot
    try:
        if hasattr(reader_client, "children"):
            kids = reader_client.children(parent_key)
            for kid in kids:
                data = kid.get("data", {}) if isinstance(kid, dict) else {}
                if (
                    kid.get("key") == attachment_key
                    or data.get("key") == attachment_key
                ):
                    break
    except Exception as exc:
        ctx.info(f"Attachment verification read encountered error (non-fatal): {exc}")

    suffix = " (stored in local Zotero storage)"
    return True, suffix, attachment_key

def trash_item(zot, item: dict):
    """Move an item to Zotero's Trash (recoverable by user) using authorized local PATCH.

    Sends a PATCH request via zot._write so Zotero-Server-ID and local Zotero-API-Key
    headers are automatically attached.
    """
    from pyzotero.zotero import build_url

    item_key = item.get("key") or item.get("data", {}).get("key")
    version = item.get("version") or item.get("data", {}).get("version", 0)

    url = build_url(
        getattr(zot, "endpoint", "http://localhost:23119/api"),
        f"/{getattr(zot, 'library_type', 'users')}/{getattr(zot, 'library_id', '0')}/items/{item_key}",
    )
    headers = {
    ...
    if hasattr(zot, "_write"):
        return zot._write("PATCH", url=url, headers=headers, content=content)
    elif hasattr(zot, "client") and hasattr(zot.client, "patch"):
        return zot.client.patch(url=url, headers=headers, content=content)
    else:
        raise RuntimeError(f"Client {zot} does not support PATCH or _write")
...
def _attach_pdf_linked_url(write_zot, pdf_url, parent_key, ctx):
    ...
    try:
        template = item_template(write_zot, "attachment", link_mode="linked_url")
